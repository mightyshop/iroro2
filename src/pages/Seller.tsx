...
const Seller: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validate form
    if (!form.title || !form.description || !form.price || !form.imageUrl || !form.category || !form.sellerName) {
  ...
  if (!isVerified) {
    return <SellerVerification onVerified={() => setIsVerified(true)} />;
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center space-x-3 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-400 mt-1">Add and manage your products</p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormInput
                label="Seller Avatar URL"
                value={form.sellerAvatar}
                onChange={(e) => setForm({ ...form, sellerAvatar: e.target.value })}
                ...
                {countries.map(country => (
                  <label
                    ...
                    className={`flex items-center p-3 rounded-lg border-2 cursor-pointer transition-colors ${
                      form.targetCountries.includes(country.value)
                        ? 'border-blue-500 bg-blue-500/10'
                  ...
                  >
                    <input
                      type="checkbox"
                      className="sr-only"
                      checked={form.targetCountries.includes(country.value)}
                      onChange={(e) => {
                        if (e.target.checked) {
                          setForm({
                            ...form,
                            targetCountries: [...form.targetCountries, country.value]
                          });
                        } else {
                          setForm({
                            ...form,
                            targetCountries: form.targetCountries.filter(code => code !== country.value)
                          });
                        }
                      }}
                    />
